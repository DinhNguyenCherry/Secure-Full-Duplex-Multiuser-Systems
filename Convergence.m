...
loadingfile = [];%['[Proposed] Convergence_7.mat'];%[];%

aux = ['-EH'];

EavsOn = 1;

MethodUsed = 4; % 0 : Proposed method
                % 1 : Conven. FD method
...
filename = ['[' Methodname{MethodUsed+1} aux '] Convergence_' num2str(FileOrder) '.mat'];
...
if (isempty(loadingfile))
    ...
    for i_NumOfSim=1:1:NumberOfRunning
        ...
        AllChannels{i_NumOfSim, 1} = CreateSmallScaleFading(1, 1, N_t, K)*D_H1k; % H1k
        AllChannels{i_NumOfSim, 2} = CreateSmallScaleFading(1, 1, N_t, L)*D_H2l; % H2l
        AllChannels{i_NumOfSim, 3} = CreateSmallScaleFading(1, 1, N_r, P)*D_G2p; % G2p
        AllChannels{i_NumOfSim, 4} = CreateSmallScaleFading(1, 1, N_r, Q)*D_G1q; % G1q
...
%         AllChannels{i_NumOfSim, 6} = CreateSmallScaleFading(1, 1, Q, L).*D_F1ql; % F1ql
        ...
        for iM = 1:1:M
            D_Hm_mat{i_NumOfSim, iM} = repmat(D_Hm(iM), N_t, N_Eavs(iM));
            AllChannels{i_NumOfSim, 7}{iM} = CreateSmallScaleFading(1, 1, N_t, N_Eavs(iM)).*D_Hm_mat{i_NumOfSim, iM}; % Hm
...
% parfor i = 1:1:length(rho_dB)
    ...
    if (SumEH)
        ...
        EH = [SumEH delta Em*ones(1,M)];
    end
    else
        EH=[];
    end
    [isBreak1, Alg1_OptValue(iXDir), Alg1_OptValueChain, OptSolution ] = ProposedAlg1( Pbs, P_up_max, AllChannel, [sigma_SI sigma_noise], MaxIteration, Fixed_Timegroup, MethodUsed, 0, eps_SCSI, EH); % P_bs = XDir(iXDir)

    
%     [isBreak1, Alg1_OptValue, Alg1_OptValueChain, OptSolution ] = ProposedAlg( Pbs, P_up_max, AllChannel, [sigma_SI sigma_noise], MaxIteration, Fixed_Timegroup, MethodUsed);
%     Alg1_OptValue = 0;
%     
%     if (isBreak1)
