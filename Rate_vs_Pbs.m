...
pathname = fileparts('./Figures/Rate_vs_Pbs/test/');
addpath(genpath('./Figures/Rate_vs_Pbs/test/'));
...
parfor iXDir = 1:1:length(XDir)
    ...
    [isBreak1, Alg1_OptValue(iXDir), Alg1_OptValueChain, OptSolution ] = ProposedAlg1( XDir(iXDir), P_up_max, AllChannel, [sigma_SI sigma_noise], MaxIteration, Fixed_Timegroup, MethodUsed, 0); % P_bs = XDir(iXDir)
%     Alg1_OptValue = 0;
%     
%     if (isBreak1)
...
% hold on
% 
% plot(Pbs_dB, Alg1_Rate, 'k-', 'linewidth', 2, 'markersize',9);
