...
pathname = fileparts('./Figures/Rate_vs_Pbs_SCSI/MaxMin_DownlinkUsers/');
addpath(genpath('./Figures/Rate_vs_Pbs/'));
...
MethodUsed = 2; % 0 : Proposed method
                % 1 : Conven. FD method
...
Methodname = {'Proposed'; 'ConvenWithoutNOMA'; 'ConvenWithNOMA'; 'HD'};

eps_SCSI = [0.99 0.99 0.99 0.99];
...
Pbs_dB = 14:4:30; % dowlink power [dBm]
...
filename = ['[' Methodname{MethodUsed+1} aux '] Rate_vs_Pbs_' num2str(FileOrder) '.mat'];
savedfile = fullfile(pathname, filename);

if (ReusedChannel)
    loadingfile = ['[' Methodname{Reusedfrom+1} aux '] Rate_vs_Pbs_' num2str(FileOrder) '.mat'];
end
...
parfor iXDir = 1:1:length(XDir)
    ...
    [isBreak1, Alg1_OptValue(iXDir), Alg1_OptValueChain, OptSolution ] = ProposedAlg1( XDir(iXDir), P_up_max, AllChannel, [sigma_SI sigma_noise], MaxIteration, Fixed_Timegroup, MethodUsed, Ru, eps_SCSI); % P_bs = XDir(iXDir)
%     Alg1_OptValue = 0;
%     
%     if (isBreak1)
...
% hold on
% 
% plot(Pbs_dB(2:2:end), Alg1_Rate, 'r-', 'linewidth', 2, 'markersize',9);
plot(Pbs_dB, Alg1_Rate, 'r-', 'linewidth', 2, 'markersize',9);
