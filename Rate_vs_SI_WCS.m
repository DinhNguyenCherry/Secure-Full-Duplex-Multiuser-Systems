...
pathname = fileparts('./Figures/Rate_vs_SI_WCS/');
% addpath(genpath('./Figures/Rate_vs_Pbs/'));
addpath(genpath('./Figures/Rate_vs_SI_WCS/'));
...
MethodUsed = 1; % 0 : Proposed method
                % 1 : Conven. FD method
...
filename = ['[' Methodname{MethodUsed+1} aux '] Rate_vs_SI_WCS_' num2str(FileOrder) '.mat'];
savedfile = fullfile(pathname, filename);

if (ReusedChannel)
    loadingfile = ['[' Methodname{Reusedfrom+1} '] Rate_vs_SI_WCS_' num2str(FileOrder) '.mat'];
end
...
parfor iXDir = 1:1:length(XDir)
    ...
    [isBreak1, Alg1_OptValue(iXDir), Alg1_OptValueChain, OptSolution ] = ProposedAlg1( Pbs, P_up_max, AllChannel, [sigma_SI(iXDir) sigma_noise], MaxIteration, Fixed_Timegroup, MethodUsed, Ru, eps_SCSI); % P_bs = XDir(iXDir)
%     Alg1_OptValue = 0;
%     
%     if (isBreak1)
...
plot(sigma_SI_dB, Alg1_Rate, 'r-', 'linewidth', 2, 'markersize',9);
